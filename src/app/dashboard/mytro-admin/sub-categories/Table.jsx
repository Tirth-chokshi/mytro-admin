...
const columns = ["No", "Name", "Banner", "Status", "Category", "Actions"];
  ...
  useEffect(() => {
    const getsubCategories = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch("http://localhost:8000/subcategories/all");
        if (!response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/subcategories/delete/${deleteCategoryId}`,
        {
          method: "DELETE",
        }
      );
      ...
      setSubCategories((prevsubcategories) =>
        prevsubcategories.filter((subCategories) => subCategories._id !== deleteSubCategoryId)
      );
    ...
    <section>
              ...
              {loading ? (
                ...
                subCategories.map((subCategory) => (
                  <TableRow key={subCategory._id}>
                    ...
                    <TableCell>
                      <Switch checked={subCategory.status} />
                    </TableCell>
                    <TableCell>
                      <Switch checked={subCategory.tempStatus} />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/dashboard/mytro-admin/sub-categories/edit/${subCategory._id}`}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
        <p className="text-sm text-center sm:text-left">
          Showing{" "}
          {subCategories.length ? (pagination.page - 1) * pagination.limit + 1 : 0}{" "}
